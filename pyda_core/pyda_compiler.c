...
static void exprbuilder_commit(ExprBuilder *builder, instrlist_t *bb, instr_t *instr, reg_id_t scratch_reg);
...
unsigned long expr_new_raw(ExprBuilder *builder, const char *buf, size_t size) {
    ...
    while (buf < end) {
        ...
        buf = decode(drcontext, (byte *)buf, instr);
        if (buf == NULL) {
...
int exprbuilder_compile(ExprBuilder *builder, instrlist_t *bb, instr_t *instr, int expr_handle_start) {
    ...
    for (unsigned long i = expr_handle_start; i < builder->exprs.entries; i++) {
        ...
        if (expr->ty == EXPR_TYPE_RAW) {
            ...
            exprbuilder_commit(builder, bb, instr, scratch_reg);
...
static void exprbuilder_commit(ExprBuilder *builder, instrlist_t *bb, instr_t *instr, reg_id_t scratch_reg) {
    ...
    for (reg_id_t reg = DR_REG_START_GPR; reg <= DR_REG_STOP_GPR; reg++) {
        ...
        if (exprbuilder_reg_get(builder, reg, &handle)) {
            ...
            DEBUG_PRINTF("exprbuilder_compile: moving final value into register %lu\n", reg);
            instr_t *new_instr = XINST_CREATE_load(drcontext, opnd_create_reg(reg), opnd_create_base_disp(scratch_reg, DR_REG_NULL, 0, 8 * handle, OPSZ_8));
            instrlist_meta_preinsert(bb, instr, new_instr);
